 ...
 * @example
 * function main() {
 *   var pBar = ProgressBar("Заголовок прогрессбара");
 *   var i;
 * 
 *   // Длинная операция #1
 ...
 *   for( i=0 ; i < 100; ++i, pBar.hit(title + " Операция: " + i) ) {
 *       $.sleep(10);
 *   }
 * 
 *   // Операция #2
 ...
 *   pBar.reset(title, 10);
 *   for( i=0 ; i < 10; ++i ) {
 *       pBar.hit(); // Без параметров еквивалентно pBar.step(i);  - c каждым вызовом hit() внутренний счётчки увеличивается на 1
 *       $.sleep(300);
 *   }
 * 
 *   pBar.close();
 * }
 */
var ProgressBar = function UIProgressBar /** @constructs ProgressBar */ (title) {
     if ( !(this instanceof UIProgressBar)) return new UIProgressBar(title);
